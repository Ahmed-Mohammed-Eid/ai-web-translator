...
function showProgressIndicator() {
	...
	progressIndicator.appendChild(
		document.createTextNode("Translating page...")
	);
		...
		translatePage(
			...
			.then((result) => {
				...
				try {
					...
					console.log(
						"Popup may be closed, but translation completed successfully"
					);
				}
			})
			.catch((err) => {
				...
				try {
					...
					console.log(
						"Popup may be closed, but there was a translation error"
					);
				}
			});
...
async function translatePage(
	...
	try {
		...
		const combinedText = textsToTranslate
			.map((item) => `[${item.index}]${item.text}[/${item.index}]`)
			.join("\n\n");
		...
		const translationMap = parseTranslations(
			translationsResult,
...
function getDirectionClass(textDirection, targetLanguage) {
	if (
		textDirection === "rtl" ||
		(textDirection === "auto" && isRtlLanguage(targetLanguage))
	) {
